...
<footer>
  <p>This page is generated from <a href="<?php echo esc_url( $json_link ); ?>">entitymap.json</a> and kept in sync with it. Published by <?php echo esc_html( $pub_name ); ?> per the EntityMap open standard.</p>
	...
	public function get_llms( $doc = null ) {
		...
		// Per the llms.txt spec the H1 must be the first element — no preceding
		// comment. Provenance is added as a trailing comment instead (see below).
		$out   = array();
		$out[] = '# ' . $publisher;
		$out[] = '';
		if ( $summary !== '' ) {
		...
		// Provenance as a trailing comment (keeps the H1 first, per the spec).
		$out[] = '<!-- Generated by BrightLocal AI Tools from the EntityMap; regenerated automatically — edits are overwritten. -->';

		return rtrim( implode( "\n", $out ) ) . "\n";
	}

	/**
	 * One markdown list item for an entity, in llms.txt style: `- [name](url): notes`.
	 * Every item is a link (spec best practice) — linked to its source page, or
	 * failing that to the entity's anchor in entitymap.html.
	 */
	private function llms_entity_line( $e ) {
		$name = isset( $e['name'] ) ? $e['name'] : '';
		$url  = $this->entity_url( $e );
		if ( $url === '' && ! empty( $e['entityId'] ) ) {
			$url = BL_EntityMap_Store::base_url() . '/entitymap.html#' . $e['entityId'];
		}
